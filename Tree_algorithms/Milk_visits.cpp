...
int depth[maxN+1], sub[maxN+1], papi[maxN+1];
vector<int>g[maxN+1];
...
void solve(){
    ...
    dfs(1, -1);
    hld(1, -1);
    map<int, vector<point>>q;
    for(int i=0 ; i<m ; i++){
        int a, b, c; cin>>a>>b>>c;
        q[c].push_back({a, b, i});
    }
    for(int i=1 ; i<=n ; i++){
        for(int it:freq[i]) st.update(repre[it], 1);
        for(point it:q[i]) ans[it.z]=path(it.x, it.y);
        for(int it:freq[i]) st.update(repre[it], 0);
    }
    string xd;
    for(int i=0 ; i<m ; i++) xd+=(ans[i]+'0');
    cout<<xd<<"\n";
}
